...
//
//  CALayer+HYAdd.m
//  StarTodayApplication3
//
//  Created by Luo liangfu on 15/12/18.
//  Copyright © 2015年 Vince. All rights reserved.
//

#import "CALayer+HYAdd.h"
#import <objc/runtime.h>

@implementation CALayer (HYAdd)

static char tagIdentifier;

-(void)setTag:(NSUInteger)tag
{
    objc_setAssociatedObject(self, &tagIdentifier, @(tag), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(NSUInteger)tag
{
    NSNumber *tagNum = objc_getAssociatedObject(self, &tagIdentifier);
    return tagNum.integerValue;
}

- (CALayer*)layerWithTag:(NSUInteger)tag
{
...
- (void)addFadeAnimationWithDuration:(NSTimeInterval)duration curve:(UIViewAnimationCurve)curve {
    ...
    [self addAnimation:transition forKey:@"hykit.fade"];
}

- (void)removePreviousFadeAnimation {
    [self removeAnimationForKey:@"hykit.fade"];
}
